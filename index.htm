<!--Meta Data======================================================= -->
<!DOCTYPE html>
<html manifest="myapp.appcache">
  <head>
    <title>Nathan Ley | Professional Developer</title>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="https://fonts.googleapis.com/css?family=Open+Sans:400,700,800" rel="stylesheet">
    <link href="./images/favicon.ico" rel="shortcut icon">
    <link href="./css/index.css" rel="stylesheet" type="text/css">
    <script src="./js/index.js" rel="script" type="text/javascript"></script>
  </head>




  <!--Parrallax Jumbotron============================================ -->  
  <body>  
    <div class="parrallax"></div>
    <div class="intro">
      <h1 class="name">Nathan Ley</h1>
      <p>_____________</p><br>
      <p>Specializing in software development and product design</p><br>
      <a href="mailto:nathanley.work@gmail.com">
        <button class="contact">Contact Me</button>
      </a>
    </div>




    <!--Step Process===================================================-->
    <div class="process">
      <h3 class="section-title">Process</h3>
      <h1 class="section-header">Keep Things Simple</h1>
      <ul>
        <li class="steps">
          <img src="./images/step-1.jpg">
          <h2>01. Strategize</h2>
          <p>Gather information for project by talking to real users and relaying data back to client to apply it to practical use </p>
        </li>

        <li class="steps">
          <img src="./images/step-2.jpg">
          <h2>02. Design</h2>
          <p>Use collected data and client input to create layout that encaptures project outlines with correct UI/UX for the all parties</p>
        </li>

        <li class="steps">
          <img src="./images/step-3.jpg">
          <h2>03. Develop</h2>
          <p>Using designs and wireframes as a guidline, carefully develop and test software to ensure quality of the product to every detail</p>
        </li>

        <li class="steps">
          <img src="./images/step-4.jpg">
          <h2>04. Launch</h2>
          <p>Wrap up and deploy software, bringing the product to life and take it to the markets for the world</p>
        </li>
      </ul>
    </div>




    <!--Client Review================================================-->
    <div class="reviews">
      <div class="wave">
        <svg class="svg" viewBox="0 0 500 150" preserveAspectRatio="none">
          <path d="M0.00,92.27 C216.83,192.92 304.30,8.39 500.00,109.03 L500.00,0.00 L0.00,0.00 Z" style="stroke: none;fill: #ffffff;"></path>
        </svg>
      </div>
      <h3 class="section-title">Clients</h3>
      <h1 class="section-header">What They Have to Say</h1>

      <div class="details">
        <h2 class="client">Greg Marinello</h2>
        <h3 class="position">CEO of Ello Consulting</h3>
        <p class="description">Working with Nathan was a clean, and efficient experience. Being a bit on the younger side, he brought a lot of new, modern designs to the table. I could always rely on him to meet deadlines, and would truly recommend him to anyone out there looking for a developer. Without him, our business would not be the same! </p>
      </div>
      <li>
        <img src="./images/ello/header.png" class="screenshots">
        <img src="./images/ello/process.png" class="screenshots">
        <img src="./images/ello/process2.png" class="screenshots">
        <img src="./images/ello/contact.png" class="screenshots">
      </li>


      <div class="details" id="space">
        <h2>Andrew Zweifel</h2>
        <h3 class="position">Co-Founder of Lame Games</h3>
        <p class="description">Detail oriented, intelligent, deadline oriented and broad-minded developer; has vast knowledge and is thorough. Thanks to interpersonal skills, Nathan has great relations with both company clients and potential customers. Is able to work in a fast-paced environment, and highly recommended.</p>   
      </div>
      <li>
        <img src="./images/lame-games/header.JPG" class="screenshots">
        <img src="./images/lame-games/login.JPG" class="screenshots">
        <img src="./images/lame-games/dashboard.JPG" class="screenshots">
        <img src="./images/lame-games/profile.JPG" class="screenshots">
      </li>
    </div>




    <!--Recommendations================================================-->
    <div class="recommendations">
      <div class="wave">
        <svg class="svg" viewBox="0 0 500 150" preserveAspectRatio="none">
          <path d="M0.00,92.27 C216.83,192.92 304.30,8.39 500.00,109.03 L500.00,0.00 L0.00,0.00 Z" style="stroke: none;fill: #f3f3f3;"></path>
        </svg>
      </div>
      <h3 class="section-title">Recommendations</h3>
      <h1 class="section-header">They Matter Too</h1>
      
      <div class="details">
        <h2 class="client">Blake Connally</h2>
        <h3 class="position">CEO of Modsi</h3><br>
        <p class="description">Nathan was awesome to work with. He went above and beyond what was required of him for the project. He is highly experienced in what he does and I would recommend him to anyone. My favorite part is he is self-managed, has great communication and delivers quality work at the end of the day. Someone like him is extremely hard to find on this platform. Highly recommended. </p>
      </div>
    </div>




    <!--Footer================================================-->
    <div class="footer">
      <div class="icons">
        <a href="https://www.linkedin.com/in/nathan-ley" target="_blank">
          <img class="item" src="./images/linkedin.png">
        </a>
        <a href="https://www.instagram.com/_nathanley" target="_blank">
          <img class="item" src="./images/instagram.png">
        </a>
        <a href="https://www.github.com/nathanleysgit" target="_blank">
          <img class="item" src="./images/github.png">
        </a>
      </div>
    </div>
  </body>
</html>
